 ...
 * ld-analyse - Configuration unit tests
 *
...
void testInstallSurvivesLongLivedWriter()
{
    ...
    // MainWindow saving its own settings, as it does when ld-analyse quits.
...
int main(int argc, char *argv[])
{
    ...
    // reads or writes the user's real ld-analyse.ini.
    QStandardPaths::setTestModeEnabled(true);
    const QString configDirectory = QStandardPaths::writableLocation(QStandardPaths::ConfigLocation);
    const QString configFile = QDir(configDirectory).filePath(QStringLiteral("ld-analyse.ini"));
