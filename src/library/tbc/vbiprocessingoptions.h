...
// VBI processing options - which data types tbc-process-vbi should decode, plus
// the teletext advanced options. Default-constructed options match the
// tbc-process-vbi CLI defaults (four in-process VBI decoders on; teletext and
// VITS off). Defined as a free type so tbc-analyse (Configuration) and the
...
struct VbiProcessingOptions {
    ...
    bool teletext = false;
    bool vits = false;
    QString teletextHtmlDir;                 // empty -> tbc-process-vbi default (<input>_teletext_html)
