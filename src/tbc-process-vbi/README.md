...
tbc-process-vbi extracts and decodes Vertical Blanking Interval (VBI) data from TBC files. It reads embedded metadata such as frame numbers, time codes, chapter marks, user codes, and closed captions, updating the TBC metadata for use by other tools.
...
### Basic Syntax
```bash
tbc-process-vbi [options] <input.tbc>
```
...
```bash
tbc-process-vbi input.tbc
```

#### Process with Multiple Threads
```bash
tbc-process-vbi -t 8 input.tbc
```

#### Don't Create Backup
```bash
tbc-process-vbi -n input.tbc
```

#### Debug Mode
```bash
tbc-process-vbi -d input.tbc
```
